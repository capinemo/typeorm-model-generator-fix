...
function removeIndicesGeneratedByTypeorm(dbModel) {
    ...
    dbModel.forEach((entity) => {
        ...
        const primaryColumns = entity.columns
            .filter((v) => v.primary)
            .map((v) => v.tscName);
        const ormTableName = (entity === null || entity === void 0 ? void 0 : entity.schema) ? `${entity.schema}.${entity.tscName}`
            : entity.tscName;
        entity.indices = entity.indices.filter((v) => !(v.primary &&
            v.name ===
                namingStrategy.primaryKeyName(ormTableName, primaryColumns)));
        entity.relations
            .filter((v) => v.joinColumnOptions)
            .forEach((rel) => {
            const columnNames = rel.joinColumnOptions.map((v) => v.name);
            const idxName = namingStrategy.relationConstraintName(ormTableName, columnNames);
            const fkName = namingStrategy.foreignKeyName(ormTableName, columnNames);
